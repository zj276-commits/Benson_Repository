...
#
# Depends on: rag.R (rag_retrieve), global.R constants, api.R/OpenAI helpers.
...
agent_date_preamble <- function() {
  paste0(
    ...
    "3. If a data field is missing or shows '--', say 'data not available' and do not estimate or guess.\n",
    "4. Every number you cite must appear in the provided data. If you cannot find it below, do not mention it.\n",
    "5. All financial data is HISTORICAL (already reported). Use past tense for fiscal-year results.\n",
    "6. Do NOT fabricate forward projections. You may describe trajectories implied by historical trends.\n",
...
AGENT_ROLES <- list(
  fundamentals = list(
    ...
    system = paste0(
      ...
      "IMPORTANT: Every number you cite must come from the provided data tables. If a metric is unavailable, state that clearly.\n\n",
  ...
  news = list(
    ...
    system = paste0(
      ...
      "Analyze both. For company news: identify earnings, guidance, M&A, product launches, and regulatory actions.\n",
      "For macro news: identify tariff policies, rate changes, trade tensions, geopolitical risks, and inflation signals, ",
      "then assess how those factors affect this company specifically.\n\n",
      "STRICT: ONLY reference events that appear in the provided news headlines. Do NOT invent, recall, or fabricate news from your training data.\n\n",
      "Return JSON with EXACTLY these keys:\n",
      ...
      "- macro_impact: 4-6 sentences on how the macro environment affects this stock specifically"
    )
  ),
  technical = list(
    ...
    system = paste0(
      ...
      "IMPORTANT: Cite the EXACT parameter values from the data. Do NOT round away the meaning or replace missing values.\n\n",
      "Return JSON with EXACTLY these keys:\n",
  ...
  bull = list(
    ...
    system = paste0(
      ...
      "- You may highlight genuine positives: improving trends, strong margins, healthy FCF, low debt, growth acceleration, and positive news from the data.\n",
  ...
  bear = list(
    ...
    system = paste0(
      ...
      "- If the data shows strong metrics, you MUST acknowledge them. Focus on genuine vulnerabilities: valuation stretch, decelerating growth, margin compression, rising capex, and competitive threats implied by the data.\n",
  ...
  risk = list(
    ...
    system = paste0(
      ...
      "- Volatility regime: realized vs 20-day and whether volatility is expanding or contracting\n",
      "- Tail risk: kurtosis, max drawdown, non-normality\n",
      "- Leverage risk: debt-to-equity trend, net debt, interest coverage\n",
      "- Idiosyncratic risk: SIM residual std and R-squared\n",
      "- Macro risks: from the provided news (tariffs, trade wars, regulation, geopolitics)\n\n",
...
PM_SYSTEM_PROMPT <- paste0(
  ...
  "- Write at institutional quality. Every paragraph must be substantive and grounded in specific analyst evidence.\n",
  ...
  "- If an analyst report is unavailable, state it is unavailable. Do NOT fill gaps with guesses.\n\n",
  "Return JSON with EXACTLY these keys:\n",
  ...
  "- industry_analysis: 10-15 sentences covering sector overview, competitive landscape, company moat, technology advantages, revenue drivers, barriers to entry, and regulation\n",
  "- investment_overview: 10-15 sentences covering valuation, price momentum, quant model interpretation, beta exposure, volatility regime, earnings quality, balance sheet strength, dividend policy, and peer comparison\n",
  "- macro_environment: 6-8 sentences analyzing the current macro/geopolitical landscape and its specific impact on this stock. Reference specific events from the news analyst's macro_impact report.\n",
...
# ----------------------------
# Enhancement 5: Multi-round RAG retry for thin context
# ----------------------------
...
detect_data_gaps <- function(agent_output) {
  ...
  gap_hits <- sum(vapply(gap_phrases, function(pattern) grepl(pattern, text, fixed = TRUE), logical(1)))
  gap_hits >= 2
}

retry_weak_agents <- function(symbol, company_name, sector, trend_text, api_key, agent_outputs) {
  ...
  if (length(needs_retry) == 0) {
    message("[Multi-RAG] All agents have sufficient context, no retry needed")
  ...
  retry_reqs <- lapply(needs_retry, function(atype) {
    ...
    user_prompt <- paste0(
      preamble, "\n",
      "Analyze ", symbol, " (", company_name, "), sector: ", sector, ".\n\n",
      "NOTE: This is a RETRY with expanded data context. Use all available data thoroughly.\n\n",
      "REMINDER: You may ONLY cite numbers and facts from the === DATA === sections below.\n\n",
      if (nzchar(trend_text)) paste0("=== CURRENT PRICE DATA ===\n", trend_text, "\n\n") else "",
  ...
  if (is.null(retry_resps)) return(agent_outputs)
...
run_parallel_agents <- function(symbol, stock_data_val, news_data_df,
  ...
  agent_outputs <- run_phase1_parallel(
    symbol = symbol,
  ...
  agent_outputs <- retry_weak_agents(symbol, company_name, sector, trend_text, api_key, agent_outputs)
